...
class ExampleApp(QtWidgets.QMainWindow, ui.Ui_MainWindow):
    ...
    def update_ships_table(self):
        ...
        for ship in dbm.get_all_ships():
            if data[ship[0]] != 0:
                ...
                self.ships_table.setItem(row_pos, 4, QtWidgets.QTableWidgetItem(ship[3]))
    ...
    def update_free_ships_table(self):
        ...
        for ship in dbm.get_all_ships():
            if data[ship[0]] == 0:
                ...
                self.free_ships_table.setItem(row_pos, 4, QtWidgets.QTableWidgetItem(ship[3]))
    ...
    def ship_in_seaport_table_update(self):
        try:
            ...
            if not items:
                return
            self.selected_port = [items[0].text(), items[1].text(), items[2].text()]
        except IndexError:
            return
...
class editShipLocDialog(QtWidgets.QDialog, edit_ship_loc.Ui_edit_ship_loc_dialog):
    def __init__(self, ship, port):
        ...
        self.ship_name = ship
        self.port_name = port
        self.current_ship.setText(self.ship_name[1] + ", Курс " + self.ship_name[2])
        self.current_loc.setText(self.port_name[1] + ", " + self.port_name[2] + " район")
